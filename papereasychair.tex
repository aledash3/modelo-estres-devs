...
\titlerunning{Modelado Estadístico del Nivel de Estrés}

\author{
David Alejandro Cruz Palacios\inst{1}
\and
Elvis Paúl García Acevedo\inst{1}
\and
Naim Andre Michelena Andrade\inst{1}
\and
Emily Mabel Ortega Constante\inst{1}
\and
Carlos José Pilatuña Roldan\inst{1}
\and
Mario Alexander Salazar Sema\inst{1}
}

\authorrunning{Cruz, García, Michelena, Ortega, Pilatuña y Salazar.}
...
Quito, Ecuador\\
\email{\{dcruzp5, egarciaa5, nmichelena, eortegac9, cpilatunar, msalazars2\}@est.ups.edu.ec}
}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.5\columnwidth]{Figuras/matriz_correlación.png}
\caption{Matriz de correlaciones de las variables de productividad y estrés.}
\label{fig:matriz_correlación}
\end{figure}
...
\begin{enumerate}
    ...
    \begin{figure}[H]
    \centering
    \includegraphics[width=\columnwidth]{Figuras/Qqplot.png}
    ...
    \begin{figure}[H]
    \centering
    \includegraphics[width=\columnwidth]{Figuras/Residuals_vs_fitted.png}
...
\subsection{Análisis de Observaciones Atípicas e Influyentes }
Se implementó un protocolo matemático estricto para mitigar el efecto de datos anómalos. Las observaciones con un residuo estandarizado superior a 2 en valor absoluto se consideraron \textit{outliers}. Para detectar observaciones con alto nivel de apalancamiento (\textit{leverage}), se calcularon los valores $h_{ii}$ de la diagonal de la matriz sombrero (\textit{Hat Matrix}), considerando problemáticos aquellos que superaron el umbral $3(p + 1)/n$, donde $p$ representa el número de variables explicativas. Finalmente, la influencia global se midió con la Distancia de Cook ($D_{i}$), penalizando observaciones con valor $D_{i} > 1$. El filtro arrojó resultados destacables: 2 observaciones atípicas estandarizadas (que se amplían a 10 bajo el umbral estudentizado), 14 con elevado \textit{leverage}, y 0 observaciones influyentes bajo el criterio estricto de Cook. Esto indica una matriz de datos estructuralmente sana.
